...
/**
  ...
  *
  *@pre Only NORMAL mtx matrices are read; don't have to be sortedg
**/
...
int main(int argc, char* argv[])
{
  ...
  cusparseSetMatIndexBase(descr_a, INDEX_BASE);
 ...
 // char const* const permFileName = argv[11];
  ...
  cusparseSpMatDescr_t jd_desc = NULL;   // create once and overwrite at each iteration
  ...
  if(jd_flag)   // if JD is not all zeros (otherwise computation is saved)
  {
    ...
    allocateMatrixOnDevice(mat_jd.m_, mat_jd.nnz_, &jd_t_i, &jd_t_j, &jd_t_v);
    
    transposeMatrixOnDevice(mat_jd.n_, 
                            mat_jd.m_, 
                            mat_jd.nnz_, 
                            jd_i, 
                            jd_j, 
    ...
    cusparseSpMatDescr_t jdtdxjd_desc = NULL;
    createCsrMat(&jdtdxjd_desc,
                 mat_jd.m_,
                 mat_jd.m_,
                 0,
                 NULL,
    ...
    int64_t C_num_rows1;
    int64_t C_num_cols1;
    int64_t jdtdxjd_nnz1;
    checkCudaErrors(cusparseSpMatGetSize(jdtdxjd_desc, 
          &C_num_rows1,
          &C_num_cols1,
          &jdtdxjd_nnz1));

    allocateMatrixOnDevice(mat_h.n_, jdtdxjd_nnz1, &jdtdxjd_i, &jdtdxjd_j, &jdtdxjd_v);
    
    checkCudaErrors(cusparseCsrSetPointers(jdtdxjd_desc,
          jdtdxjd_i,
          jdtdxjd_j,
          jdtdxjd_v));
    ...
    checkCudaErrors(cusparseSpMatGetSize(jd_t_desc, &a_n, &a_m, &a_nnz));
    allocateVectorOnDevice(a_nnz, &a_v);
    checkCudaErrors(cusparseSpMatGetValues(jd_t_desc, (void**)(&a_v)));
    //displayDeviceVector(a_v, a_nnz, 10, "Values of Product Matrix 1");
    
    //displayDeviceVector(jdtdxjd_v, jdtdxjd_nnz1, 10, "Product");

    //------------------------------------------------------------

    /* It's time for the sum Htilde= H + (J_d^TD_xJ_d)
    nnz_total_devhost_ptr points to host memory
     Allocation for matrix addition - happens once
    */
    size_t buffer_byte_size_add;
    void* buffer_add            = NULL;
    int*  nnz_total_devhost_ptr = &nnz_htil;
    allocateVectorOnDevice(mat_h.n_ + 1, &htil_i);
    ...
    cusparseXcsrgeam2Nnz(handle, 
                         ...
                         descr_a, 
                         htil_i,
                         nnz_total_devhost_ptr, 
                         buffer_add);
    
    nnz_htil = *nnz_total_devhost_ptr;
    
    allocateVectorOnDevice(nnz_htil, &htil_j);
    allocateVectorOnDevice(nnz_htil, &htil_v);
    ...
    //displayDeviceVector(htil_v, nnz_htil, 10, "Htil");

    gettimeofday(&t2, 0);
    timeM += (1000000.0 * (t2.tv_sec - t1.tv_sec) + t2.tv_usec - t1.tv_usec) / 1000.0;
  //free on last iteration
  deleteMatrixOnDevice(jdtdxjd_i, jdtdxjd_j, jdtdxjd_v);
  ...
  deleteOnDevice(buffer_add);
#else    
    // Compute H_til= H+J_d^T * D_s * J_d
    SpgemmClass* sc = new SpgemmClass(mat_h.n_, handle, ONE, ONE, ONE);
    
    sc->load_product_matrices(mat_jd.m_, jd_t_desc, jd_s_desc);
    sc->allocate_product();
    sc->compute_product();
    return 0;
    sc->load_sum_matrices(h_i,
        h_j,
        h_v,
        mat_h.nnz_);
    sc->allocate_sum(); 
    ...
    nnz_htil = sc->getResultMatrix(&htil_i, &htil_j, &htil_v);
    ...
    matrixDeviceToDeviceCopy(mat_h.n_, mat_h.nnz_, h_i, h_j, h_v, htil_i, htil_j, htil_v);
  ...
  transposeMatrixOnDevice(mat_jc.n_, 
                          mat_jc.m_, 
                          mat_jc.nnz_, 
                          jc_i, 
                          jc_j, 
  ...
  allocateMatrixOnDevice(mat_jc.m_, mat_jc.nnz_, &jct_i_c, &jct_j_c, &jct_v_c);
  ...
  RuizClass rz(ruiz_its, mat_h.n_, n_hj);
  ...
  int     nnz_hgam;
    ...
    cusparseSpMatDescr_t jct_jc_desc = NULL;
    createCsrMat(&jct_jc_desc,
          mat_jc.m_,
          mat_jc.m_,
          0,
          NULL,
    ...
    int64_t C_num_cols1;
    int64_t jctjc_nnz1;
    checkCudaErrors(cusparseSpMatGetSize(jct_jc_desc, 
          &C_num_rows1,
          &C_num_cols1,
          &jctjc_nnz1));

    allocateMatrixOnDevice(mat_jc.m_, jctjc_nnz1, &jctjc_i, &jctjc_j, &jctjc_v);
    
    checkCudaErrors(cusparseCsrSetPointers(jct_jc_desc,
          jctjc_i,
          jctjc_j,
          jctjc_v));
    ...
    //displayDeviceVector(jctjc_v, jctjc_nnz1, 10, "Product");

  /* It's time for the sum Hgamma= Htilde + gamma(J_c^TJ_c)
  nnz_total_devhost_ptr2 points to host memory
   Allocation for matrix addition - happens once*/
  size_t  buffer_byte_size_add2;
  char*   buffer_add2 = nullptr;
  int*    nnz_total_devhost_ptr2 = &nnz_hgam;
  allocateVectorOnDevice(mat_h.n_ + 1, &hgam_i);
  cusparseDcsrgeam2_bufferSizeExt(handle, 
                                  mat_h.n_, 
                                  mat_h.n_, 
                                  &ONE, 
                                  ...
                                  hgam_i, 
                                  hgam_j, 
                                  &buffer_byte_size_add2);
  allocateVectorOnDevice(buffer_byte_size_add2, &buffer_add2);
  ...
  cusparseXcsrgeam2Nnz(handle, 
                       ...
                       descr_a, 
                       hgam_i,
                       nnz_total_devhost_ptr2, 
                       buffer_add2);

  nnz_hgam = *nnz_total_devhost_ptr2;
  allocateVectorOnDevice(nnz_hgam, &hgam_j);
  allocateVectorOnDevice(nnz_hgam, &hgam_v);
  // Matrix addition - happens every iteration
  gettimeofday(&t1, 0);
  cusparseDcsrgeam2(handle, 
                    mat_h.n_, 
  ...
  deleteMatrixOnDevice(jctjc_i, jctjc_j, jctjc_v);
  ...
  deleteOnDevice(buffer_add2);
#else
  SpgemmClass* sc = new SpgemmClass(mat_h.n_, handle, gamma, ONE, ONE);
    
  sc->load_product_matrices(mat_jc.m_, jc_t_desc, jc_desc);
  sc->allocate_product();
  sc->compute_product();
  sc->load_sum_matrices(htil_i,
                        htil_j,
                        htil_v,
                        nnz_htil);
  sc->allocate_sum(); 
  ...
  nnz_hgam = sc->getResultMatrix(&hgam_i, &hgam_j, &hgam_v);
  // This closes the if J_d!=0 statement
  delete sc;
#endif  
  double* d_rx_hat;
  ...
  PermClass pc(mat_h.n_, nnz_hgam, mat_jc.nnz_);
  ...
  CholeskyClass cc (mat_h.n_, nnz_hgam, hgam_p_val, hgam_i, hgam_j);
  cc.Analysis();
  ...
  SchurComplementConjugateGradient sccg(jc_p_desc, 
                                        jc_t_descp, 
                                        d_y, 
                                        d_schur, 
                                        mat_jc.n_, 
